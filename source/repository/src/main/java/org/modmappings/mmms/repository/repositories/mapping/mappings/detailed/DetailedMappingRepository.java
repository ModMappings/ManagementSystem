...
public interface DetailedMappingRepository extends org.springframework.data.repository.Repository<DetailedMappingDMO, UUID> {
    ...
    /**
     * Finds all mappings of which the input or output matches the given expression.
     ...
     * the given id.
     *
     * This method, compared to {@link org.modmappings.mmms.repository.repositories.mapping.mappings.mapping.MappingRepositoryCustom#findAllOrLatestFor(Boolean, UUID, UUID, MappableTypeDMO, String, String, UUID, UUID, UUID, UUID, UUID, boolean, Pageable)}
     ...
     * @return All latest mappings, and their metadata, who' matches the given expressiones and are part of the mapping type and game version if those are specified.
     */
     ...
     */
    Mono<DetailedMappingDMO> findBy(final UUID id,
                                    final boolean externallyVisibleOnly);
}
