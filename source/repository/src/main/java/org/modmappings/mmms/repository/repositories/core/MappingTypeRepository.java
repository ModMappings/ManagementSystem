...
public class MappingTypeRepository extends ModMappingR2DBCRepository<MappingTypeDMO> {
    ...
    public Mono<Page<MappingTypeDMO>> findAllBy(
        ...
        return createPagedAllBy(nameRegex, editable, pageable);
    }

    private Mono<Page<MappingTypeDMO>> createPagedAllBy(
            final String nameRegex,
            final Boolean editable,
            final Pageable pageable
    ) {
                ...
                selectSpec -> selectSpec
                    .withCriteria(
                            where(
                                    ...
                                    .and(parameter(editable)).isNull().or(reference( "editable")).is(parameter(editable))
                    ),
                pageable
        );
    }
