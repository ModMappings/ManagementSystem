...
using API.Model.Read.Core.Proposal;
...
namespace API
{
        ...
        where TReleaseEntry : AbstractReleaseMember<TMapping, TVersionedMapping, TCommittedEntry, TProposalEntry, TReleaseEntry>
    {
        ...
        {
            var dbModels = (await _readerWriter.AsQueryable()).Skip(pageSize * pageIndex).Take(pageSize);
        ...
        {
            return Content((await (await _readerWriter.AsQueryable()).CountAsync()).ToString());
        }
        ...
        {
            var currentProposal = await _readerWriter.GetProposalMapping(proposalId);
        ...
        {
            var committedMapping = await _readerWriter.GetLiveMapping(committedMappingId);
