    ...
    public static class DbMigrationHelpers
    {
        ...
        where TRole : IdentityRole, new()
        {
            using (var scope = serviceProvider.GetRequiredService<IServiceScopeFactory>().CreateScope())
            {
                ...
                await EnsureSeedIdentityData(rootConfiguration.SeedConfiguration, userManager, roleManager);
            }
        }
        ...
        private static async Task EnsureSeedIdentityData<TUser, TRole>(
            ISeedConfiguration seedConfiguration,
            ...
            where TRole : IdentityRole, new()
        {
            ...
            // Create admin user
            if (await userManager.FindByNameAsync(seedConfiguration.AdminUserSeedConfiguration.AdminUserName) != null) return;

            var user = new TUser
            {
                UserName = seedConfiguration.AdminUserSeedConfiguration.AdminUserName,
                Email = seedConfiguration.AdminUserSeedConfiguration.AdminEmail,
                EmailConfirmed = true
            };

            var result = await userManager.CreateAsync(user, seedConfiguration.AdminUserSeedConfiguration.AdminPassword);
